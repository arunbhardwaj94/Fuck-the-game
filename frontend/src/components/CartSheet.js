...
import { Minus, Plus, Trash2, ShoppingBag, MessageCircle, MapPin, ChevronRight } from 'lucide-react';
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4" data-testid="checkout-view">
            ...
            <div className="bg-gray-50 rounded-xl p-3 space-y-2">
              ...
              {cart.items.map(item => (
                <div key={item.product_id} className="flex justify-between text-sm font-body">
                  ...
                  <span className="font-heading font-bold">&#8377;{item.item_total}</span>
                </div>
              ))}
              <div className="border-t border-gray-200 pt-2 mt-2 flex justify-between">
                <span className="font-heading font-bold text-gray-900">Total</span>
                <span className="font-heading font-extrabold text-lg text-gray-900">&#8377;{cart.total}</span>
              </div>
            </div>
            ...
            <div>
              ...
              {addresses.length > 0 ? (
                <div className="space-y-2">
                  {addresses.map(addr => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-xl border-2 transition-colors ${selectedAddressId === addr.id ? 'border-brand-green bg-brand-green-light' : 'border-gray-100 bg-white'}`}
                ...
                <div className="space-y-2">
                  <Input
                    data-testid="quick-address-input"
                    placeholder="Enter delivery address..."
                    value={quickAddress}
                    onChange={e => setQuickAddress(e.target.value)}
                    className="h-12 rounded-xl text-base font-body"
                  />
                  <button onClick={() => { setCartOpen(false); navigate('/profile'); }} className="text-xs text-brand-green font-body font-medium hover:underline" data-testid="save-address-link">
            ...
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="font-heading font-bold text-sm text-gray-700">Payment: Cash on Delivery</p>
            </div>
          </div>
        ) : (
          /* CART ITEMS VIEW */
